...
public class AddIssueServiceBackUp extends Service
{
    ...
    public int onStartCommand(Intent intent, int flags, int startId)
    {
        ...
        PendingIntent cancelPendingIntent = PendingIntent.getBroadcast(mContext, (int) System.currentTimeMillis(), cancelIntent, 0);
        ...
        PendingIntent retryPendingIntent = PendingIntent.getBroadcast(mContext, (int) System.currentTimeMillis(), retryIntent, 0);
    ...
    private class UploadAsyncTask extends AsyncTask<Void, Void, String>
    {
        ...
        protected void onPostExecute(String response)
        {
            ...
            mNotifyManager.notify(NOTIFICATION_ID, mBuilder.build());
            AddIssueServiceBackUp.this.stopSelf();
        }
        ...
        String upload()
        {
            ...
            try
            {
                ...
                entity.addPart(ResultListener.ACTION, get(ACTION_ADD_ISSUE));
                //TODO allow multiple file uploading
                entity.addPart(IssuesDao.NO_OF_IMAGES, get("1"));
                entity.addPart(IssuesDao.USER_ID, get(mIssue.userId));
                entity.addPart(IssuesDao.USER_DP_URL, get(mIssue.userDpUrl));
                entity.addPart(IssuesDao.USERNAME, get(mIssue.username));
                entity.addPart(IssuesDao.DESCRIPTION, get(mIssue.description));
                entity.addPart(IssuesDao.AREA_TYPE, get(mIssue.areaType));
                entity.addPart(IssuesDao.RADIUS, get(mIssue.radius+""));
                entity.addPart(IssuesDao.LATITUDE, get(mIssue.latitude));
                entity.addPart(IssuesDao.LONGITUDE, get(mIssue.longitude));
